# Organizations and Operating World

## Jack's advisory practice

A discreet private special-situations practice serving wealthy families, estates, closely held companies, and clients facing problems conventional firms cannot solve. It provides Jack legitimate access, income, reputation, and referrals. Exact legal entity name, staffing, and public website remain unresolved.

## Jack's residence

A South End townhouse in Boston, owned by Jack Mallory in his own name. Tasteful, private, understated. Not a crew gathering place. This is the sanctuary Jack goes to when even the crew is too much. Interior details remain unresolved and should be locked before the first on-page scene set there.

## The operating base

A working warehouse on the Chelsea waterfront. Purchased cash years ago and held in the name of a long-standing alias built by Nina; the alias predates the property acquisition and has independent filing, credit, tax, and utility history sufficient to survive routine records inspection. Jack Mallory does not appear anywhere on the title chain, and no legal entity connected to his advisory practice is on record against the address.

The building looks industrial from the outside and is kept that way. Loading dock, freight elevator, fenced yard, vehicle bay large enough to accept the Suburban and close the door behind it. Interior built out for operations: tech room for Nina, secure records storage, gear lockers, an occasional workbench for Anton, open floor for Reese to rig practice climbs, small kitchen, two couches. No pretensions to living space.

**Who knows the address:** the permanent five only. The base predates Anton and Reese joining the crew; each of them was brought there for the first time as a deliberate moment of trust. The address is not shared with clients, referrals, family members (including Betty), or professional contacts.

**Alias name:** `[TBD — lock before any on-page use].`

The alias-held title is baseline crew infrastructure and predates every operation in the current series. In the scale of what the permanent five have done together over sixteen years, the title fraud is trivial and should not be treated in-story as a meaningful additional exposure. It matters only insofar as it is one more thing Hattie could eventually connect to Jack.

## The permanent crew

A financially secure, long-standing found family with specialized operational authority. The crew may accept payment but is not driven primarily by financial need.

## Lowell Security Group

Marcus Lowell's legitimate security-consulting business. It supports executive protection, risk work, transport, and lawful security services while providing credible infrastructure for crew activity.

## Anton's locksmith and security shop

A working ground-floor storefront in Brighton, Boston, on a commercial stretch with genuine Russian and Eastern European neighborhood texture. Painted wood sign, gold-leaf letters on the glass, single word: `LOCKSMITH`. No business name displayed. The shop handles keys, safes, security hardware, restoration, and specialized mechanical systems for a mix of walk-in customers and long-established referrals. Katya's cello may be heard from the small residence above or behind the shop.

Legal name of the business, if any, remains unresolved.

## Stan's bookstore

A struggling used bookstore in Boston. It is genuinely a bookstore and also supports discreet finding, authentication, movement, or arrangement of unusual objects and information.

## Federal Bureau of Investigation

Hattie Baker operates inside the FBI as a patient, principled investigator. Her Book One involvement begins through an unrelated financial investigation touching a Weaver Newman client.

## AM Strategic Advisory, LLC

**Public name:** AM Advisory  
**Location:** Washington, D.C. area  
**Leader:** Addison Moody

### Public services

- Crisis management.
- Litigation intelligence.
- Corporate investigations.
- Cyber response.
- Reputation management.
- Political risk.
- Security.

### Hidden special-matters structure

A compartmentalized criminal capability serving powerful clients. Three unnamed senior advisers form part of the hidden leadership beneath Addison. Rylan Adams is a mid-level associate, not inner circle.

Methods include bribery, surveillance, cyber intrusion, coercion, evidence manipulation, lawfare, political pressure, and murder.

## Weaver, Newman & Gaines, LLC

**Common shorthand:** Weaver Newman or WNG
**Book One location:** Restored Boston Mercantile Trust Building

The firm occupies a historic banking hall and original vault complex. Relevant areas include:

- Restored public banking hall and ballroom/gala space.
- Restricted office floors (WNG occupies floors six through nine).
- Original main vault chamber.
- Deposit compartments.
- Internal document-control cabinet with an antique secondary lock.
- Archive level.
- Service elevator network.
- Rear service area reachable through an abandoned counterweight shaft.
- Preserved bank-officers' stair from the executive corridor directly down to the vault level, badge-controlled. Retained during the restoration as a historic feature and left functional. Ordinarily locked at the corridor door; the badge reader accepts only credentials on the facilities whitelist.

## Boston Mercantile Trust Building

Book One's primary operational location. The building's layered historic and modern systems create the impossible-access problem and the later fire/extraction problem.

### Vault access system (canon)

The original vault door was preserved and remains mechanically functional (seven-foot restored steel, brass wheel). A modern access layer was added during the WNG restoration:

- Six-digit keypad beside the door, keyed to a small authorized-user set that includes senior partners and a facilities backup list.
- Ceiling camera above the door; feeds to building security.
- Access logs record vault entries and are reviewed on an after-the-fact basis by building security. There is no live alerting inside the firm's ordinary operational hours or after them unless the door is forced.

### Chelsea Boone's contributions to Stephen's entry

Chelsea's role in the prologue is not technical live-monitoring. Her contributions are the kind of quiet administrative acts an internal systems auditor can plausibly commit and defend:

- She added Stephen's badge to the facilities whitelist months earlier, under cover of a documented audit test, so his badge would open the bank-officers' stair door.
- She and Stephen together identified when Tomas Weaver had used his six-digit code in Stephen's presence and confirmed it privately.
- She and Stephen reconstructed the internal vault layout from an archived renovation permit she pulled through her ordinary systems access.
- She provisioned a simple cryptic communication utility across their personal devices for short pre-arranged messages. The utility is not live telemetry. It cannot show her what Stephen is doing inside the vault. Its only intended use for the prologue night is a single one-word confirmation text — *Home* — that Stephen was to send to Chelsea after he was clear of the building.

Chelsea did not loop cameras. Chelsea did not bypass sensors. Chelsea did not run an admin console. During Stephen's entry Chelsea has no visibility into the vault, the corridor, or the building. She is at home waiting for the text.
